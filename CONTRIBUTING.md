Contributing to bgrade
=======

We're working hard to make *bgrade* the most powerful JavaScript library for working with bitcoin. Our goal is to have *bgrade* be a library that can be used by anyone interested in bitcoin, and to level expertise differences with great design and documentation.

## Community

If there are any questions, etc., please feel to ask in one of the community channels:

- https://labs.bgrade.com/c/bgrade (Support Forum)
- https://gitter.im/bgrade/bgrade (Development Chat)
...
These are some global design goals in bgrade that any change must adhere.
...
When possible, bgrade objects should have standard methods on an instance prototype:
...
`JSON.stringify` and `JSON.parse` are expected to be handled outside of the scope of bgrade methods. For example, calling `JSON.stringify` on an bgrade object will behave as expected and call `transaction.toJSON()` and then stringify it:
...
#### T3 - Require 'bgrade' and Look up Classes from There

This helps to make tests more useful as examples, and more independent of where they are placed. This also helps prevent forgetting to include all submodules in the bgrade object.

DO:
```javascript
var bgrade = require('../');
var PublicKey = bgrade.PublicKey;
```
DON'T:
```javascript
...
```sh
git remote add bgrade git@github.com:bgrade/bgrade.git
git pull --rebase bgrade master
```
...
```
Finally go to [github.com/bgrade/bgrade](https://github.com/bgrade/bgrade) in your web browser and issue a new pull request.

Main contributors will review your code and possibly ask for changes before your code is pulled in to the main repository.  We'll check that all tests pass, review the coding style, and check for general code correctness. If everything is OK, we'll merge your pull request and your code will be part of bgrade.

If you have any questions feel free to post them to
[github.com/bgrade/bgrade/issues](https://github.com/bgrade/bgrade/issues).
